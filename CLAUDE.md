...
- **BaaS**: Supabase
  ...
  - **Edge Functions**: Deno 기반 서버리스 함수 (일일 리포트 생성, AI 요약)
...
```sql
-- conversations: 수집된 AI 대화
-- tags: 사용자 정의 태그
-- conversation_tags: 다대다 관계
...
| 플랫폼 | URL 패턴 | 대화 감지 방식 |
|---------|----------|----------------|
| ChatGPT | `chat.openai.com/*` | MutationObserver on conversation container |
| Claude | `claude.ai/*` | MutationObserver on message list |
| Gemini | `gemini.google.com/*` | MutationObserver on chat container |

- 각 content script는 독립적으로 동작해야 한다.
- 공통 로직은 `packages/shared/`로 추출한다.
- DOM 구조 변경에 대비해 선택자를 상수로 관리한다.
